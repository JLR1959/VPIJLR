...
/* ======================================================
MODULE PROTECTION FERMETURE
ANTI PERTE DONNÉES
====================================================== */

window.addEventListener("beforeunload", function (e) {

  // Vérifier s’il y a du contenu (ex: dossier en cours)
  const locataire = document.getElementById("locataire")?.value;

  if (locataire && locataire.trim() !== "") {
    e.preventDefault();
    e.returnValue = "";
  }
...
// ==========================
// AFFICHER CLIENTS CLOUD
// ==========================
const sectionCloud = document.getElementById("section-clients-cloud");
if(sectionCloud){
  sectionCloud.style.display = "block";
}
...
// ==========================
// MASQUER CLIENTS CLOUD
// ==========================
const sectionCloud = document.getElementById("section-clients-cloud");
if(sectionCloud){
  sectionCloud.style.display = "none";
}
const controles = document.getElementById("admin-controles");
const login = document.getElementById("zone-login-admin");
const champ = document.getElementById("admin-password");
...
setTimeout(() => {
  ...
  if(typeof afficherClientsCloud === "function"){
    afficherClientsCloud();
  }
...
window.cloudAPI.ajouterClient(client)
.then(()=>console.log("✔ Cloud sauvegardé"))
...
/* ======================================================
MODULE 201
MODE LOCATAIRE / PROPRIETAIRE (SERVEUR)
====================================================== */
...
const cle = localStorage.getItem("cleLicence");

if(!cle){
console.log("Aucune licence");
return;
}

const res = await fetch("https://licence-server-jlr-eui5.onrender.com/verifier-acces",{
method:"POST",
...
/* 🔥 MODE SELON LICENCE */
if(data.licence && data.licence.mode === "locataire"){

activerModeLocataire();

}else{

activerModeProprietaire();

}

}catch(e){

console.log("Mode inconnu (offline)");
...
/* ======================================================
MODULE 206
PANNEAU ADMINISTRATEUR LICENCE (SANS JSON)
====================================================== */

function afficherInfosLicence(){

const cle = localStorage.getItem("cleLicence") || "Non définie";
...
<br>
<button onclick="document.getElementById('panneau-admin-licence').remove()">
Fermer
</button>
`;
...
/* ======================================================
MODULE 175
STATUT LICENCE VISUEL
====================================================== */

async function verifierEtatLicence(){
    const barre = document.getElementById("etatLicence");
    if (!barre) return;

    const cle = localStorage.getItem("cleLicence");
    if (!cle) {
        barre.textContent = "Licence : NON CONFIGURÉE";
        barre.style.background = "#b54708";
        return;
    }

    try {
        const res = await fetch("https://licence-server-jlr.onrender.com/verifier-acces", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({ cle })
        });
        ...
        if (data.autorise) {
            barre.textContent = "Licence : ACTIVE";
            barre.style.background = "#027a48";
        } else {
            barre.textContent = "Licence : DÉSACTIVÉE";
            barre.style.background = "#b42318";
        }
    } catch (e) {
        barre.textContent = "Licence : HORS LIGNE";
        barre.style.background = "#b54708";
    }
}
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    setInterval(verifierEtatLicence, 60000);
});
...
function sauvegarderLicence(){
    const input = document.getElementById("inputLicence");
    if (!input) return;

    const cle = input.value.trim();
    if (!cle) {
        alert("Entrer une licence");
        return;
    }

    localStorage.setItem("cleLicence", cle);
    alert("Licence enregistrée");
}

/* ======================================================
MODULE 175
CHARGER LICENCE AU DEMARRAGE
====================================================== */
...
/* ======================================================
MODULE 800
LICENCE NON BLOQUANTE (FIX FINAL)
====================================================== */
...
/* 🔥 NE BLOQUE PLUS */
if(!cle){
console.log("Aucune licence - accès autorisé");
afficherEtatLicence("Aucune licence", "#b54708");
return;
}
...
console.log("Licence désactivée - accès autorisé");
afficherEtatLicence("Licence désactivée", "#b42318");
return;
}

/* OK */
afficherEtatLicence("ACTIVE", "#027a48");

}catch(e){

console.log("Mode hors ligne");
afficherEtatLicence("Hors ligne", "#b54708");
...
function afficherEtatLicence(texte, couleur){

const barre = document.getElementById("etatLicence");

if(!barre) return;

barre.textContent = "Licence : " + texte;
...
// ======================================================
// MODULE 28 - LISTE CLIENTS CLOUD + CHARGEMENT
// ======================================================

// ==========================
// AFFICHER LISTE CLIENTS
// ==========================
async function afficherClientsCloud(){

  if(!window.cloudAPI) return;

  try{

    const clients = await window.cloudAPI.chargerClients();
    ...
    clients.slice().reverse().forEach(client => {
      ...
      div.innerHTML = `
        <strong>${client.locataire || "Sans nom"}</strong><br>
        ${client.adresse || ""}<br>
        ${client.date || ""}
      `;

      div.onclick = () => chargerClientDansFormulaire(client);
    ...
    console.error("Erreur affichage clients", e);
  }
}

// ==========================
// CHARGER CLIENT DANS FORMULAIRE
// ==========================
function chargerClientDansFormulaire(client){

  document.getElementById("locataire").value = client.locataire || "";
  document.getElementById("telephone").value = client.telephone || "";
  document.getElementById("numeroAppartement").value = client.appartement || "";
  document.getElementById("adresse").value = client.adresse || "";
  document.getElementById("ville-quebec").value = client.ville || "";
  ...
  document.getElementById("verificateur").value = client.verificateur || "";
  document.getElementById("verification-date").value = client.date || "";
